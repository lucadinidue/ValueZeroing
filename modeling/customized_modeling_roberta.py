...
class RobertaSelfAttentionVZ(nn.Module):
    ...
    def forward(
        ...
        ## Added for Value-Zeroing

        if zero_value_index is not None:  # zeroing value vectors corresponding to the given token index
            value_layer[:, :, zero_value_index] = torch.zeros(value_layer[:, :, zero_value_index].size())
...
class RobertaLayerVZ(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = RobertaAttentionVZ(config, position_embedding_type="absolute")
...
class RobertaForMaskedLMVZ(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        prediction_scores = prediction_scores[torch.arange(prediction_scores.size(0)), target_index]  # Added for Value-Zeroing
